  ...
  $.get(s3_rest_url)
    .done(function(data) {
      ...
      if (info.nextMarker !== 'null') {
        ...
        // render breadcrumbs
        var nav_html = '<ol class="breadcrumb">';
...
function buildNavigation(info) {
  ...
  jQuery.each(directories, function(index, directory) {
    if (index === 0) { // root directory
      directoryName = '<span class="fas fa-home"></span>';
    ...
    if (index === directories.length - 1) { // last directory
      html_list.push('<li class="breadcrumb-item active">' + directoryName + '</li>\n'); // don't display as link
    } else {
      html_list.push('<li class="breadcrumb-item"><a href="?prefix=' + directoryPath + '">' + directoryName + '</a></li>\n');
    }
  });
